...
public class ALLitDetailGUI : ShaderGUI
{
    ...
    private static class Styles
    {
        ...
        public static string debugingMode = "DebugTexturesMode";
        ...
        public static readonly string[] debugNames = { "FinalColor", "BaseColor", "SpecularF0", "Metalness", "Smoothness", "Normal", "Occlusion", "EmssionColor" };

        // texture
        public static GUIContent baseMapText = new GUIContent("Albedo", "Albedo (RGB) and Transparency (A)");
        public static GUIContent normalMapText = new GUIContent("Normal", "NormalTangentSpace Toggle: OpenGl/DX");
        public static GUIContent msaeMapTex = new GUIContent("UnityMaskMap", "Metallic (R) Smoothness (G) Occlusion(B) EmissMask(A) Toggle: Unity/Unreal");
        public static GUIContent emissMapTex = new GUIContent("EmissionMap", "EmissionMap(RGB)");
        public static GUIContent smoothnessRemapText = new GUIContent("GlossRef");
        public static GUIContent queueSlider = new GUIContent("Priority", "RenderQueue Offset");

        public static GUIContent detailMaskMap = new GUIContent("DetailMask", "DetailMask For Secondary Map(R)");


        public static GUIContent detailAlbedoMap = new GUIContent("DetailAlbedo", "Albedo(RGB) Blend");
        public static GUIContent detailNormalMap = new GUIContent("DetailNormal", "DetailNormal Blend ");
        public static GUIContent detailAlbedoMapScale = new GUIContent("DetailAlbedoScale");
        public static GUIContent detailNormalMapScale = new GUIContent("DetailNormalScale");
        ...
        public static GUIContent directdiffuseText = EditorGUIUtility.TrTextContent("DirectDiffuse", "Direct Diffuse");
        public static GUIContent indirectdiffuseText = EditorGUIUtility.TrTextContent("IndirectDiffuse", "Indirect Diffuse");
    ...
    MaterialProperty DetailAlbedoMapScale = null;
    ...
    MaterialProperty directdiffuse = null;
    MaterialProperty indirectdiffuse = null;
    ...
    public void FindProperties(MaterialProperty[] props)
    {
        ...
        DetailMaskMap = FindProperty("_DetailMaskMap", props);
        ...
        directdiffuse = FindProperty("_DirectDiffuse", props);
        indirectdiffuse = FindProperty("_IndirectDiffuse", props);
    ...
    public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] props)
    {
        ...
        EditorGUILayout.Space();
        GUILayout.Label("Options Freeze Don't Click", EditorStyles.boldLabel);
        DebugModePopup();
        // Third properties
        if (directdiffuse != null)
            m_MaterialEditor.ShaderProperty(directdiffuse, Styles.directdiffuseText);
        if (highlights != null)
            m_MaterialEditor.ShaderProperty(highlights, Styles.highlightsText);
        if (indirectdiffuse != null)
            m_MaterialEditor.ShaderProperty(indirectdiffuse, Styles.indirectdiffuseText);
        if (reflections != null)
    ...
    void RenderMode(Material material)
    {
        ...
        SetupMaterialWithDebugMode(material, (DebugMode)debugMode.floatValue);
    ...
    public void DebugKeyWolrd(Material material, bool final, bool basec, bool f0, bool meta, bool smooth, bool normal, bool occ, bool emiss)
    {
        SetMaterialKeyword(material, "_FINALCOLOR_ON", final);
        SetMaterialKeyword(material, "_BASECOLOR_ON", basec);
        SetMaterialKeyword(material, "_SPECULARF0_ON", f0);
        SetMaterialKeyword(material, "_METALNESS_ON", meta);
        SetMaterialKeyword(material, "_SMOOTHNESS_ON", smooth);
        SetMaterialKeyword(material, "_NORMALMAP_ON", normal);
        SetMaterialKeyword(material, "_OCCLUSION_ON", occ);
        SetMaterialKeyword(material, "_EMISSION_ON", emiss);
    }
    public void SetupMaterialWithDebugMode(Material material, DebugMode debugMode)
    {
        switch (debugMode)
        {
            case DebugMode.FinalColor:
                DebugKeyWolrd(material, true, false, false, false, false, false, false, false);
                break;
            case DebugMode.BaseColor:
                DebugKeyWolrd(material, false, true, false, false, false, false, false, false);
                break;
            case DebugMode.SpecularF0:
                DebugKeyWolrd(material, false, false, true, false, false, false, false, false);
                break;
            case DebugMode.Metalness:
                DebugKeyWolrd(material, false, false, false, true, false, false, false, false);
                break;
            case DebugMode.Smoothness:
                DebugKeyWolrd(material, false, false, false, false, true, false, false, false);
                break;
            case DebugMode.Normal:
                DebugKeyWolrd(material, false, false, false, false, false, true, false, false);
                break;
            case DebugMode.Occlusion:
                DebugKeyWolrd(material, false, false, false, false, false, false, true, false);
                break;
            case DebugMode.EmssionColor:
                DebugKeyWolrd(material, false, false, false, false, false, false, false, true);
                break;
        }
    }
    ...
    public void ShaderPropertiesGUI(Material material)
    {
        ...
        {
            m_MaterialEditor.TexturePropertySingleLine(Styles.normalMapText, normalMap, normalScale, normalMapSwitch);
        ...
        if (normalMap.textureValue != null || DetailNormalMap.textureValue != null)
        {
        ...
        {
            Styles.msaeMapTex.text = "UnrealBPRMap";
            Styles.msaeMapTex.tooltip = "Occlusion (R) Roughness (G) Metallic(B) EmissMask(A) Toggle: Unity/Unreal";
            Styles.smoothnessRemapText.text = "RoughRef";
        }
        else
        {
            Styles.msaeMapTex.text = "UnityBPRMap";
            Styles.msaeMapTex.tooltip = "Metallic (R) Smoothness (G) Occlusion(B) EmissMask(A) Toggle: Unity/Unreal";
        ...
        m_MaterialEditor.TexturePropertySingleLine(Styles.detailMaskMap, DetailMaskMap);

        if (DetailAlbedoMap.textureValue != null)
        {
            m_MaterialEditor.TexturePropertySingleLine(Styles.detailAlbedoMap, DetailAlbedoMap, DetailAlbedoMapScale);
        }
        else
        {
            m_MaterialEditor.TexturePropertySingleLine(Styles.detailAlbedoMap, DetailAlbedoMap);
        }
        //Detail Normal
        if (DetailNormalMap.textureValue != null)
        {
            m_MaterialEditor.TexturePropertySingleLine(Styles.detailNormalMap, DetailNormalMap, DetailNormalMapScale);
        }
        else
        {
            m_MaterialEditor.TexturePropertySingleLine(Styles.detailNormalMap, DetailNormalMap);
        }

        var detailTiling = DetailAlbedoMap.textureScaleAndOffset;
        material.SetTextureScale("_DetailAlbedoMap", new Vector2(detailTiling.x, detailTiling.y));
        material.SetTextureOffset("_DetailAlbedoMap", new Vector2(detailTiling.z, detailTiling.w));
        m_MaterialEditor.TextureScaleOffsetProperty(DetailAlbedoMap);

        if (DetailAlbedoMap.textureValue != null || DetailNormalMap.textureValue != null)
        {
            SetMaterialKeyword(material, "_DETAIL_SCALED", true);
        }
        else
        {
            SetMaterialKeyword(material, "_DETAIL_SCALED", false);
        }
        m_MaterialEditor.ShaderProperty(uvSetSecondary, "UVSet");
    ...
    void DebugModePopup()
    {
        ...
        EditorGUI.BeginChangeCheck();
        mode = (DebugMode)EditorGUILayout.Popup(Styles.debugingMode, (int)mode, Styles.debugNames);
